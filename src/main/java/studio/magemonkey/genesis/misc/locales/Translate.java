...
package studio.magemonkey.genesis.misc.locales;
...
public final class Translate extends JavaPlugin {
    ...
    public static Locale getLocale(Player p) {
        Locale standard = Locale.valueOf(ClassManager.manager.getPlugin().getConfig()
                .getString("Locales", "en_us").replace("-", "_").toLowerCase()).createIfNotExists();
        if (standard == null) {
            standard = Locale.en_us.createIfNotExists();
        }
        if (p == null) {
            return standard;
        }
    ...
    public static String getVillager(@NotNull Locale locale, @NotNull Object career) {
        Preconditions.checkArgument(career.getClass().getSimpleName().equals("Career"),
                "Are you using Villager.Career ???");
    ...
    public static String getTropicalFish(@NotNull Locale locale, @NotNull TropicalFish.Pattern type) {
        return TranslationRegistry.get(locale)
                .getValue("entity.minecraft.tropical_fish.type." + type.name().toLowerCase());
    }
}
